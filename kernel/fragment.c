...
#define structure(S,m,p) ((S *)( (u8_t *)(p) - (u8_t *) &((S *)0)->m ))
/*
#define ANIMATE
 */
...
typedef struct fh {
	u8_t         used;           // this position in table in use
	fragkey      key;
...
extern u16_t  _pktipofs;  /* offset from header to start of pkt */
...
u8_t *fragment( struct ip_header * ip )
{
	fraghdr    *my_frag;
...
// Check if we have a match
	...
	if (!found) {
	    ...
	    *((u8_t *)ip - (2 + _pktipofs)) = 2;
	    ...
	    my_frag->data_offset = (u8_t *)my_frag->ip + in_GetHdrlenBytes(ip);
	    // Setup initial hole table
	    if (data_start) {// i.e. not Zero
		memcpy(my_frag->data_offset + data_start,(u8_t *)ip+in_GetHdrlenBytes(ip),data_length);
...
// Hole handling
	...
	if (!my_frag->hole_first) {
	    ...
	    return((u8_t *)my_frag->ip - _pktipofs);
	}
#ifdef ANIMATE
	printf( " MORE_TO_COME\n" );/*DEBUG STUFF*/
#endif
